...
public class SelectTeam_pmw {
	...
	public int solution(int[] scores, int k) {
		...
		// intervalNode[]는 scores 최대길이 - 1
		// 마지막 node는 다음 node와의 interval 계산 필요없음
		...
		// IntervalNode 내림차순 정렬(IntervalNode.compareTo)
		Arrays.sort(intervalDiffs);
		System.out.println("intervalDiffs = " + Arrays.toString(intervalDiffs));

		// scores배열 데이터 ArrayList 세팅
		List<Integer> scoresList = new ArrayList<Integer>();
		for (int i : scores) {
			scoresList.add(i);
		}

		// 점수사이에 가장 간격이 큰 데이터 순으로 자를 그룹의 index 세팅
		int[] splitIndexs = new int[k - 1];
		for (int i = 0; i < k - 1; i++) {
			splitIndexs[i] = intervalDiffs[i].index + 1;
		}
		Arrays.sort(splitIndexs);
		System.out.println("splitIndexs = " + Arrays.toString(splitIndexs));
		...
		for (int i = 0; i < splitIndexs.length; i++) {
			...
			if (beforeIdx == cutIdx) {
				...
				// subList 요소 가장 마지막값에서 첫번째값을 뺀 값을 answer에 더한다
	...
	public static void main(String[] args) {
		SelectTeam_pmw test2 = new SelectTeam_pmw();
